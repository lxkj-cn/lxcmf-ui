    ...
    <el-card class="top">
        <el-row>
            <el-col :span="6" class="tools">
                <el-input v-model="obj.formData.keyword" placeholder="请输入公告名称" clearable autocomplete="off"></el-input>
        ...
        <el-row>
            <el-col :span="2" class="tools">
                <ButtonPlus type="primary" name="notice_add" @clickSuccess="add"></ButtonPlus>
            </el-col>
        </el-row>
        <el-table :data="obj.listData" border stripe @sort-change="sortChange" style="width: 100%;margin-top:20px">
            <el-table-column type="selection" width="55" />
            <el-table-column prop="notice_id" label="编号" width="80" />
            <el-table-column prop="title" label="公告名称" width="280" />
            ...
            <el-table-column label="是否置顶" :sortable="`custom`" prop="is_top" width="120">
                <template v-slot="scope">
            ...
            <el-table-column fixed="right" label="操作">
                <template v-slot="scope">
                    <el-button-group>
                        <ButtonPlus type="primary" size="small" name="notice_edit" @clickSuccess="edit(scope.row)">
                        </ButtonPlus>
                        <ButtonPlus type="danger" size="small" name="notice_del" @clickSuccess="del(scope.row)">
                        </ButtonPlus>
                    </el-button-group>
                </template>
            </el-table-column>
...
const getList = () => {
    apis.contentApis.getNoticeListPage(formData).then(res => {
        if (res.meta.code === 1) {
...
const updateSort = (row) => {
    updateData({ notice_id: row.notice_id, sort: row.sort })
}
const updateStatus = (row) => {
    updateData({ notice_id: row.notice_id, status: row.status })
}
const updateTop = (row) => {
    updateData({ notice_id: row.notice_id, is_top: row.is_top })
}
const updateData = (newData) => {
    apis.contentApis.saveNoticeEdit(newData).then(res => {
        if (res.meta.code === 1) {
...
const edit = (row) => {
    router.push('/notice/Edit?notice_id=' + row.notice_id)
}
const del = (row) => {
    ElMessageBox.confirm(
        ...
        apis.contentApis.saveNoticeDel({ notice_id: row.notice_id }).then(res => {
            if (res.meta.code === 1) {
                ElMessage.success(res.meta.msg)
                getList()
            } else {
